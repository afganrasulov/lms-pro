...
export default function AdminLogsPage() {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 h-[calc(100vh-140px)] flex flex-col">
            <div>
                <h1 className="text-3xl font-bold tracking-tight text-white mb-2">Sistem Logları</h1>
                <p className="text-slate-400">Son sistem aktiviteleri ve kullanıcı işlemleri.</p>
            </div>
            ...
            <Card className="bg-slate-900 border-slate-800 text-white flex-1 overflow-hidden flex flex-col">
                <CardHeader className="bg-slate-950 border-b border-slate-800 py-3">
                    <div className="flex items-center">
                        ...
                        <CardTitle className="text-base">Aktivite Akışı</CardTitle>
                        <Badge variant="outline" className="ml-auto border-slate-700 text-slate-400">Canlı</Badge>
                    </div>
                </CardHeader>
                <CardContent className="p-0 flex-1 relative">
                    {loading ? (
                        <div className="absolute inset-0 flex items-center justify-center text-slate-400">
                            <Loader2 className="w-6 h-6 animate-spin mr-2" /> Loglar yükleniyor...
                        </div>
                    ) : logs.length === 0 ? (
                        <div className="absolute inset-0 flex items-center justify-center text-slate-400">
                            Log bulunamadı.
                                    ...
                                    <div key={log.id} className="p-4 flex items-start gap-4 hover:bg-slate-800/30 transition-colors text-sm">
                                        ...
                                        <div className="flex-1 space-y-1">
                                            ...
                                            <div className="text-xs text-slate-500 flex items-center gap-2">
                                                ...
                                                <span>•</span>
                                                <span>{formatDistanceToNow(new Date(log.created_at), { addSuffix: true, locale: tr })}</span>
                                            </div>
                                        </div>
                                    </div>
                                ))}
